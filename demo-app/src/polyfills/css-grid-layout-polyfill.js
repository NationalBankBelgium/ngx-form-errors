...
!(function() {
	...
	module.exports = (function(window, document) {
		...
		function consumeASimpleBlock(s) {
			...
			while (s.consume()) {
				if (s.token instanceof EOFToken || (s.token instanceof GroupingToken && s.token.value == mirror)) return block;
				else {
					s.reconsume();
			...
			polyfillStyleInterface: function(cssPropertyName) {
				var prop = {
					get: function() {
						...
						try {
							if (!this.parentElement)
								throw new Error("Please use the anHTMLElement.myStyle property to get polyfilled properties");
						} catch (ex) {
							setImmediate(function() {
								throw ex;
							});
					...
					set: function(v) {
						...
						try {
							if (!this.parentElement)
								throw new Error("Please use the anHTMLElement.myStyle property to set polyfilled properties");
						} catch (ex) {
							setImmediate(function() {
								throw ex;
							});
				...
				cssCascade.startMonitoringRule(
					cssSyntax.parse("[data-style-" + cssPropertyName + "]{" + cssPropertyName + ":attr(style)}").value[0]
				);
	...
	module.exports = (function(window, document) {
		...
		GridItem.prototype = {
			...
			updateFromElement: function() {
				...
				this.hPaddings =
					parseInt(usedStyle.getPropertyValue("padding-left")) + parseInt(usedStyle.getPropertyValue("padding-right"));
				this.vPaddings =
					parseInt(usedStyle.getPropertyValue("padding-top")) + parseInt(usedStyle.getPropertyValue("padding-bottom"));
				this.hBorders =
					parseInt(usedStyle.getPropertyValue("border-left-width")) + parseInt(usedStyle.getPropertyValue("border-right-width"));
				this.vBorders =
					parseInt(usedStyle.getPropertyValue("border-top-width")) + parseInt(usedStyle.getPropertyValue("border-bottom-width"));
			...
			parseLocationInstructions: function(specifiedStart, specifiedEnd, cssText) {
				...
				gridColumnStart: while (true) {
					if (value[I] instanceof cssSyntax.IdentifierToken) {
						...
						// this is wrong
						console.error(
							"INVALID DECLARATION: grid-column/row: " + value.toCSSString() + " (no token to analyze before the slash token)"
						);
						this.buggy = true;
						return;
					} else {
				...
				if (value[I]) {
					if (value[I] instanceof cssSyntax.DelimToken && value[I].value == "/") {
						...
						console.error(
							"INVALID DECLARATION: grid-column/row: " + value.toCSSString() + " (expected slash / or end of declaration)"
						);
						this.buggy = true;
						return;
					}
		...
		GridLayout.prototype = {
			...
			updateFromElement: function() {
				...
				var usedStyle = style;
				this.hlPadding =
					parseInt(usedStyle.getPropertyValue("border-left-width")) + parseInt(usedStyle.getPropertyValue("padding-left"));
				this.hrPadding =
					parseInt(usedStyle.getPropertyValue("border-right-width")) + parseInt(usedStyle.getPropertyValue("padding-right"));
				this.vtPadding =
					parseInt(usedStyle.getPropertyValue("border-top-width")) + parseInt(usedStyle.getPropertyValue("padding-top"));
				this.vbPadding =
					parseInt(usedStyle.getPropertyValue("border-bottom-width")) + parseInt(usedStyle.getPropertyValue("padding-bottom"));
			},
			...
			parseTrackBreadth: function(value, I) {
				...
				var parseTrackBreadthToken = function() {
					...
					console.error(
						"INVALID DECLARATION: grid-template-rows/columns: " + value.toCSSString() + " (unrecognized track breadth)"
					);
					buggy = true;
					return;
				};
					...
					if (value[I] instanceof cssSyntax.SimpleBlock && value[I].name == "(") {
						...
						for (var J = tokens.length; J--; ) {
							if (tokens[J] instanceof cssSyntax.IdentifierToken) {
								...
								console.error(
									"INVALID DECLARATION: grid-template-rows/columns: " + value.toCSSString() + " (unrecognized line name)"
								);
								buggy = true;
								return;
							}
			...
			computeAbsoluteTrackBreadths: function() {
				...
				var computeTrackBreadthIncrease = function(
					...
					var rows_and_limits = xSizes.map(function(item, cx) {
						return {
							...
							limit: xSizes[cx].limit,
							minIsMinContent:
								specifiedSizes[cx].minType == TRACK_BREADTH_MIN_CONTENT || specifiedSizes[cx].minType == TRACK_BREADTH_AUTO,
							minIsMaxContent: specifiedSizes[cx].minType == TRACK_BREADTH_MAX_CONTENT,
							maxIsMinContent: specifiedSizes[cx].maxType == TRACK_BREADTH_MIN_CONTENT,
							maxIsMaxContent:
								specifiedSizes[cx].maxType == TRACK_BREADTH_MAX_CONTENT || specifiedSizes[cx].maxType == TRACK_BREADTH_AUTO
						};
					});
					rows_and_limits.sort(function(a, b) {
						return a.limit - b.limit;
				...
				var computeFinalTrackBreadth = function(
					...
					computeTrackBreadth.call(this, xSizes, this_xSizes, getMinWidthOf, getMaxWidthOf, getXStartOf, getXEndOf);
				...
				var getMinHeightOf = function(item) {
					return item.element.offsetHeight + item.vMargins - Math.max(0, item.yEnd - item.yStart - 1) * rowGap;
				};
